...
# Default number of MPI tasks if not specified
NUM_TASKS=4

# Check if number of tasks is provided as argument
if [ $# -eq 1 ]; then
    NUM_TASKS=$1
    echo "Using $NUM_TASKS MPI tasks"
else
    echo "Using default $NUM_TASKS MPI tasks (specify as argument to change)"
fi
...
declare -a tests=(
    "1 2 3 10 1:1.570 0.056:0" #1
    "1 1 5 100 1:0.786 0.027:0" #2
    "-10 3 3 100 10:1.733 1.643:2" #3
    "-50 10000 200 500000 10:5025.384 5303.928:4" #4
    "1 5000 100 1000000 4:3936.506 131.472:4" #5
    "1 5000 100 1000000 100:3899.787 156.291:163" #6
    "1 5000 20 1000000 10:3918.912 143.364:19" #7
    "1 1000 3 10000 10000:287.788 261.446:31" #8
    "3 5000 50 1000000 300:3819.032 25.659:469" #9
    "3 5000 50 1000000 500:3738.436 58.743:804" #10
    "-1 1000 30 100000 1000:575.878 370.663:1203" #11
)
...
for test in "${tests[@]}"; do
    ...
    echo "Test $total_tests: mpirun -np $NUM_TASKS ./parsim-mpi $input"
    
    # Run the program with mpirun and capture output and time
    start_time=$(date +%s.%N)
    output=$(mpirun -np $NUM_TASKS ./parsim-mpi $input)
    exit_code=$?
    end_time=$(date +%s.%N)
    execution_time=$(echo "$end_time - $start_time" | bc)
    
    if [ $exit_code -ne 0 ]; then
        echo -e "${RED}Test $total_tests Failed: Program crashed with exit code $exit_code${NC}"
        continue
    fi
